<script>
    $(document).ready(() => {
        $("#parameter_kode_rekening_saldo_table").DataTable({
            responsive: true,
            ...
            ajax: "<?= base_url("/index.php/api/parameter_kode_rekening_saldo_cabang/$data_branch->id") ?>",
            columns: [{
                    data: 'id',
    ...
    function konfirmasi_hapus(id) {
        Swal.fire({
            ...
            if (result.value) {
                window.location.href = `<?= base_url("/index.php/api/delete_parameter_kode_rekening_saldo/") ?>${id}`;
            }
        })
    }
</script>