<script>
    $(document).ready(() => {
        $("#parameter_neraca_saldo_akhir_table").DataTable({
            responsive: true,
            ...
            ajax: "<?= base_url("/index.php/api/parameter_neraca_saldo_akhir_cabang/$data_branch->id") ?>",
            columns: [{
                    data: 'id',
    ...
    function konfirmasi_hapus(id) {
        Swal.fire({
            ...
            if (result.value) {
                window.location.href = `<?= base_url("/index.php/api/delete_parameter_neraca_saldo/") ?>${id}`;
            }
        })
    }
</script>